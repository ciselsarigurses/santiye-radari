...
saha yanlış-pozitifi, tarla/şerit negatif bağlamı veya spektral FP-klon baskısı
mevcut route-gate çıktısından aynen miras alınır.
"""
...
def _temporal_support(candidate, scene_date, precursors):
    ...
    precursor_date = _date(precursor.get("son_guclu_tarih") or precursor.get("ilk_guclu_tarih"))
...
def _analyze_region(region_key, route_region, precursors):
    ...
    for candidate in route_region.get("adaylar") or []:
        ...
        blocked = bool(
            candidate.get("geri_bildirim_engeli") is True
        ...
        rows.append({
            ...
            "temporal_oncul_mesafe_m": round(precursor_distance, 1) if precursor_distance is not None else None,
            ...
            "uc_eksenli_destek": bool(morphology_ok and sar_positive and temporal_positive),
            ...
            "yanlis_pozitif_spektral_klon_baskilandi": candidate.get("yanlis_pozitif_spektral_klon_baskilandi") is True,
            "alarm": False,
            "saha_gorevi": False,
        })
    ...
    return {
        ...
        "uc_eksenli_destekli_sayi": sum(1 for item in rows if item["uc_eksenli_destek"]),
        "adaylar": rows,
    }
...
def audit(route=None, temporal=None, sar_calibration=None):
    ...
    sar_calibration = (
        sar_calibration if isinstance(sar_calibration, dict) else _load(SAR_CAL_JSON)
    )
    precursors = [
        item for item in (temporal.get("adaylar") or [])
    ...
    total_main = sum(x["ana_esik_pozitif_destekli_sayi"] for x in regions.values())
    total_micro = sum(x["mikro_pozitif_destekli_sayi"] for x in regions.values())
    total_three = sum(x["uc_eksenli_destekli_sayi"] for x in regions.values())
    sar_veto_available = sar_calibration.get("sar_zorunlu_veto_kullanilabilir") is True
    return {
        "surum": 1,
        "amac": "SAR yokluğunu veto yapmadan morfoloji + S1 pozitif veya önceki-tarih temporal-lokal kanıtını çaprazlamak",
        ...
        "bolgeler": regions,
        "not": (
            "Bu çıktı yalnız recall diagnostiğidir. SAR güçlü-lokal desteği pozitif kanıttır; SAR yokluğu "
            "negatif veto değildir. Önceki tarih temporal-lokal iz yalnız mevcut morfoloji adayıyla 25 m "
            "içinde ve daha eski bir sahnede ise ikinci kanıt sayılır. 250 m² ana eşik ve 150–249 m² MİKRO "
            "politikası değişmez; bu katman otomatik saha görevi üretmez."
        ),
    }
...
def _self_check():
    route = {
        "bolgeler": {
            "cesme": {
                ...
                "adaylar": [
                    {"enlem": 38.30, "boylam": 26.30, "spektral_etki_alani_m2": 400, "morfoloji_puani": 90, "morfoloji_yuksek": True, "sar_capraz_destek": False},
                    {"enlem": 38.31, "boylam": 26.31, "spektral_etki_alani_m2": 400, "morfoloji_puani": 90, "morfoloji_yuksek": True, "sar_capraz_destek": False},
                    {"enlem": 38.32, "boylam": 26.32, "spektral_etki_alani_m2": 500, "morfoloji_puani": 88, "morfoloji_yuksek": True, "sar_capraz_destek": True},
                    {"enlem": 38.33, "boylam": 26.33, "spektral_etki_alani_m2": 200, "morfoloji_puani": 85, "morfoloji_yuksek": True, "sar_capraz_destek": False},
                    {"enlem": 38.34, "boylam": 26.34, "spektral_etki_alani_m2": 400, "morfoloji_puani": 95, "morfoloji_yuksek": False, "sar_capraz_destek": False, "yanlis_pozitif_spektral_klon_baskilandi": True},
                    {"enlem": 38.35, "boylam": 26.35, "spektral_etki_alani_m2": 400, "morfoloji_puani": 95, "morfoloji_yuksek": True, "sar_capraz_destek": False, "geri_bildirim_engeli": True},
                ],
            },
    ...
    temporal = {
        "adaylar": [
            {"temporal_iz_id": "T1", "enlem": 38.30001, "boylam": 26.30001, "bolge_anahtari": "cesme", "son_guclu_tarih": "15.09.2026"},
            {"temporal_iz_id": "T2", "enlem": 38.33001, "boylam": 26.33001, "bolge_anahtari": "cesme", "son_guclu_tarih": "15.09.2026"},
            {"temporal_iz_id": "T3", "enlem": 38.34001, "boylam": 26.34001, "bolge_anahtari": "cesme", "son_guclu_tarih": "15.09.2026"},
            {"temporal_iz_id": "T4", "enlem": 38.35001, "boylam": 26.35001, "bolge_anahtari": "cesme", "son_guclu_tarih": "15.09.2026"},
        ]
    }
