...
def _output_shape(bbox, max_width=1050):
    ...
    width_km = (east - west) * 111.32 * np.cos(np.radians(mean_lat))
    height_km = (north - south) * 110.57
    height = max(320, round(max_width * height_km / max(width_km, 0.1)))
    return min(height, 850), max_width
...
def _reflectance(raw):
    # Sentinel-2 Collection 1 L2A bant ölçeği ve radyometrik ofseti.
...
def _clean_mask(mask):
    """Tek piksellik gürültüyü komşuluk çoğunluğu ile azaltır."""
...
def analyze_sentinel_change(region_key, pair=None):
    ...
    vegetation_loss = _ndvi(older_red, older_nir) - _ndvi(latest_red, latest_nir)
    ...
    change_mask = _clean_mask(soil_signal | strong_visual_change)
    ...
    west, south, east, north = bbox
    pixel_width_m = (east - west) * 111320 * np.cos(
